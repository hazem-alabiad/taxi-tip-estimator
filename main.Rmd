...
head(dt.predicted)
```
## Predicted vs original tip percentage
```{r}
head(as.data.frame(dt.predicted) %>% cbind(test.set$tip_percentage))
```
## Error rate in predicting the tip percentage using Decision Tree (DT)
```{r}
cat("Error rate is ~ (", round(sum(dt.predicted != test.set$tip_percentage) / nrow(test.set), digits = 3), " %)\n")
cat("Success rate is ~ (", round(sum(dt.predicted == test.set$tip_percentage) / nrow(test.set), digits = 3), " %)\n")
```
As we can see from above, if we accept only 3 digits after the point, which may cause some lose in data thus error, we obtained a good success rate using RT.
//////////////////////////////////////////////////////////////////////////
...
print(proc.time() - st)
```
## Error rate in predicting the tip percentage using SVM
```{r}
head(as.data.frame(svm.predicted) %>% cbind(test.set$tip_percentage))
```
```{r}
cat("Error rate is ~ (", round(sum(svm.predicted != test.set$tip_percentage) / nrow(test.set), digits = 3), " %)\n")
cat("Success rate is ~ (", round(sum(svm.predicted == test.set$tip_percentage) / nrow(test.set), digits = 3), " %)\n")
```
```{r}
x <- seq_along(test.set$tip_percentage)
plot(x, test.set$tip_percentage, pch=18, col="red")
lines(x, svm.predicted, lwd="1", col="blue")
