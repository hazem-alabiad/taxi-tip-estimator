...
```
```{r}
dt.mse <- svm.mse(test.set$tip_percentage, dt.predicted)
dt.mae <- svm.mae(test.set$tip_percentage, dt.predicted)
dt.rmse <- svm.rmse(test.set$tip_percentage, dt.predicted)
...
print(proc.time() - st)
mean(abs(nn.pred$tip_percentage))
```
## Load NN model
```{r}
